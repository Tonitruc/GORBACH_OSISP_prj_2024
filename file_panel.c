...
MITEM** load_dir(FILE_PANEL* file_panel) {
    ...
    while(temp != NULL) {
        for(int j = 0; j < FILE_MENU_FORMAT_COL; j++, num++) {
            if(j == 0) {
                my_items[num] = init_menu_item(temp->data->file_name); 
                switch (temp->data->file_type)
                {
                case DIRECTORY: {my_items[num]->color = MENU_YELLOW; my_items[num]->pnt_sym = '/'; break; }
                case PDF: {my_items[num]->color = MENU_RED; my_items[num]->pnt_sym = ' '; break; }
                case DOC: {my_items[num]->color = MENU_BLUE; break; }
                case IMAGE: {my_items[num]->color = MENU_GREEN; break; }
                case ARCHIVE: {my_items[num]->color = MENU_GRAY; break;}
                default: my_items[num]->color = MENU_WHITE; my_items[num]->pnt_sym = '*';
                }
            }
            else if(j == 1) {
                char buffer[20];
                sprintf(buffer, "%ld", temp->data->size_kb);
                my_items[num] = init_menu_item(cstowchs(buffer));    
            }
            else if(j == 2) {
                my_items[num] = init_menu_item(wtime(temp->data->edit_time));   
            }
        }
        temp = temp->next;    
    }
